...
//         if (!answer || !question.correctAnswer || question.type === 'long-text') return false;
//
//         const userParts = answer.split(',').map(p => p.trim().toLowerCase());
//         const correctParts = question.correctAnswer.split(',').map(p => p.trim().toLowerCase());
//
...
//     }, [answer, question, isSubmitted]);
//
//     if (question.type === 'instruction') {
//         return <div className="quiz-instruction">{question.instruction || question.text}</div>;
//     }
//
//     if (question.type === 'image') {
//         return (
//             <div className="quiz-image">
//                 <img src={question.image || question.src} alt={question.alt || "Quiz diagram"} />
//             </div>
//         );
//     }
//
...
//
// // --- Main App ---
// const TechnicalMathematicsP1Nov2021 = () => {
...
//         QUIZ_DATA.forEach(q => {
//             if (!q.correctAnswer || !userAnswers[q.id] || q.type === 'long-text') return;
//
//             const userParts = userAnswers[q.id].split(',').map(p => p.trim().toLowerCase());
//             const correctParts = q.correctAnswer.split(',').map(p => p.trim().toLowerCase());
//
...
//     };
//
//     const maxScore = useMemo(() => QUIZ_DATA.reduce((sum, q) => sum + (q.points || 0), 0), []);
//
//     return (
...
// };
//
// export default TechnicalMathematicsP1Nov2021;
